...
export const SearchDialog = () => {
	...
	return (
		<Dialog.Root open={open} onOpenChange={setOpen}>
			...
			/>
			<Dialog.Portal>
				<Dialog.Popup className='fixed top-44 left-1/2 w-2xl -translate-x-1/2 rounded-md border border-surface-400 bg-surface-100 p-4 shadow-2xl shadow-surface-400/50 focus:outline-none'>
									...
									>
										<p
											className={cn('font-bold text-lg', {
												'opacity-30': note.title.length === 0,
											})}
										>
											{note.title.length > 0 ? note.title : 'Untitled'}
										</p>
										<p className='line-clamp-2 text-typography-500 text-xs'>
											{note.body}
										</p>
									</Link>
								</li>
