...
It's a single TanStack Start app with a flat `src/`: `components/`, `hooks/`, `lib/` (utils, auth client, schemas, query-list helpers), `prosemirror/` (the editor's prosemirror extensions), `db/` (drizzle schemas and connection), `routes/` (file-based routes), and `server/` (server functions and middleware).
